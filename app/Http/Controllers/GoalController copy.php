        ...
        foreach ($request->input('milestones', []) as $mData) {
            ...
            foreach ($mData['tasks'] ?? [] as $tData) {
                ...
                if (!empty($tData['status_id'])) {
                    ...
                    if (!$wasCompletedBefore && $task->completed_at) {
                        \App\Services\PointsService::complete($task);
                    }
                    elseif ($wasCompletedBefore && !$task->completed_at) {
                        \App\Services\PointsService::uncomplete($task);
                    }
                }
            }
